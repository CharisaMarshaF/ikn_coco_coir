...
@section('content')
            ...
            <div class="col-span-12 lg:col-span-8">
                    ...
                    <div class="w-full flex-1">
                        <div class="relative text-slate-500">
                            <input type="text" id="searchProduk" onkeyup="filterProduk()"
                                ...
                                placeholder="Cari produk berdasarkan nama atau jenis...">
                ...
                <div class="grid grid-cols-12 gap-3 mt-5" id="produkList">
                    @foreach ($produk as $p)
                        ...
                        <div class="item-produk col-span-6 sm:col-span-4 lg:col-span-3 box p-3 cursor-pointer zoom-in relative border-2 border-transparent hover:border-primary"
                            ...
                            <div class="text-primary font-black text-xs">
                                Rp {{ number_format($p->harga_default, 0, ',', '.') }}
                            </div>

                            <div class="mt-2 pt-2 border-t border-slate-100 flex justify-between items-center">
                                <div class="text-xs font-bold ">
                                     Stok :<span
                                        class="text-xs font-bold {{ $stokSekarang <= 5 ? 'text-danger' : 'text-success' }}"> {{ (float) $stokSekarang }} {{ $p->satuan }}</span>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <script>
        ...
        function renderCart() {
            ...
            if (cart.length === 0) {
                container.innerHTML =
                    `<div class="text-center py-5 text-slate-400 italic" id="empty-cart">Belum ada produk dipilih.</div>`;
...
<div class="mb-4 bg-slate-50 p-3 rounded-lg border border-slate-200 intro-x">
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <div>
            ...
            <input type="text" value="${formatRupiah(item.harga)}" 
                   ...
                   onchange="updateItem(${index}, 'harga', unformatRupiah(this.value))" 
    ...
    <div class="text-right text-xs mt-2 font-bold text-primary">Subtotal: Rp ${subtotal.toLocaleString('id-ID')}</div>
        ...
        // Fungsi memformat angka menjadi format ribuan (1.000.000)
...
// Fungsi membersihkan titik agar menjadi angka murni
function unformatRupiah(rupiah) {
    return parseFloat(rupiah.replace(/\./g, '')) || 0;
}
    </script>
@endsection
