...
@section('content')
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <button data-tw-toggle="modal" data-tw-target="#modal-tambah-bahan" class="btn btn-primary shadow-md mr-2">Tambah
                Bahan Baku</button>

            <a href="{{ route('stock-log.index', ['type' => 'bahan_baku']) }}"
                        ...
                        <tbody>
                            @foreach ($bahan as $key => $b)
                                <tr class="intro-x">
                                    ...
                                    <td class="text-">
                                        <div
                                            class="font-bold {{ ($b->stok->jumlah ?? 0) <= 5 ? 'text-danger' : 'text-success' }}">
                                    ...
                                    <td class="table-report__action w-56">
                                        <div class="flex justify- items-center">
                                            <a class="flex items-center mr-3 text-primary" href="javascript:;"
                                                data-tw-toggle="modal" data-tw-target="#modal-edit-{{ $b->id }}">
                                                <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> Edit
                                            </a>
                                            <a class="flex items-center mr-3 text-danger" href="javascript:;"
                                                data-tw-toggle="modal" data-tw-target="#modal-delete-{{ $b->id }}">
                                                <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Delete
                                            </a>
                                            <a href="{{ route('stock-log.index', ['item_id' => $b->id, 'type' => 'bahan_baku']) }}"
                        ...
                        <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                            ...
                            <div class="col-span-12 sm:col-span-6">
                                ...
                                <select name="satuan" class="form-select" required>
                                    @foreach (['Kg', 'Gram', 'Liter', 'Pcs', 'Meter'] as $satuan)
                                        <option value="{{ $satuan }}" {{ $b->satuan == $satuan ? 'selected' : '' }}>
                                            {{ $satuan }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-span-12 sm:col-span-6">
                                ...
                                <input type="text" class="form-control bg-slate-100"
                                    value="{{ (float) ($b->stok->jumlah ?? 0) }}" readonly>
                            </div>
                    ...
                    <div class="modal-body p-0">
                        <form action="{{ route('bahan-baku.destroy', $b->id) }}" method="POST">
                            ...
                            <div class="p-5 text-center">
                                ...
                                <div class="text-slate-500 mt-2">Data bahan baku <b>{{ $b->nama }}</b> akan dihapus
                                    secara permanen.</div>
                            </div>
