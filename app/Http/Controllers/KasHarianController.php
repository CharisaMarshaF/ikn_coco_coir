...
class KasHarianController extends Controller
{
    public function index(Request $request)
    {
        ...
        // 2. LOGIKA DEFAULT: Jika rekening_id kosong, ambil rekening pertama
        if (!$rekening_id) {
            $firstRekening = Rekening::first();
            if ($firstRekening) {
                return redirect()->route('kas.index', [
                    'rekening_id' => $firstRekening->id,
                    'tgl_mulai' => $tgl_mulai,
        ...
        // 3. Query utama (Hanya filter berdasarkan rekening yang dipilih)
        $query = KasHarian::with(['details', 'kategoriKas', 'rekening' => function ($q) {
            $q->withTrashed();
        }])
            ->whereBetween('tanggal', [$tgl_mulai, $tgl_selesai])
            ->where('rekening_id', $rekening_id); // Rekening WAJIB ada sekarang

        if ($kategori_kas_id) {
            $query->where('kategori_kas_id', $kategori_kas_id);
        }

        $kas = $query->orderBy('tanggal', 'desc')->orderBy('created_at', 'desc')->get();

        // 4. Data pendukung
        $rekenings = Rekening::all();
        $kategoris = KategoriKas::all();

        // Statistik khusus untuk rekening yang dipilih saja (Agar Dashboard Akurat)
        $totalMasukBulanIni = KasHarian::where('rekening_id', $rekening_id)
            ->whereMonth('tanggal', date('m'))
            ->whereYear('tanggal', date('Y'))
            ->where('jenis', 'masuk')
            ->sum('total_nominal');

        $totalKeluarBulanIni = KasHarian::where('rekening_id', $rekening_id)
            ->whereMonth('tanggal', date('m'))
            ->whereYear('tanggal', date('Y'))
            ->where('jenis', 'keluar')
            ->sum('total_nominal');

        // Ambil nama rekening aktif untuk judul
        $currentRekening = Rekening::find($rekening_id);
        $title = 'Kas Harian - ' . ($currentRekening->nama ?? 'Tidak Ditemukan');
